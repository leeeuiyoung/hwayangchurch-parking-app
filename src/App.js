...
function QueryPage({ db, userId, setDbError, appId, geminiApiKey }) {
    ...
    const downloadExcel = () => {
        ...
        const headers = ["기록일시", "주차일", "이름", "직분", "주차장소", "주차시간(시간)", "시간당요금(원)", "계산된요금(원)", "계좌정보"];
        let csvContent = "\uFEFF" + headers.join(",") + "\r\n";
        results.forEach(record => {
            const row = [ formatDateTime(record.createdAt?.toDate() || record.parkingDate), record.parkingDate, record.name, record.position, record.parkingLocation, record.parkingDurationHours + (record.isCustomDuration ? ` (${record.customDurationDetail})` : ''), record.hourlyRate, record.calculatedFee, record.accountInfo ].map(escapeCsvCell).join(",");
            csvContent += row + "\r\n";
        });
        csvContent += "\r\n";
